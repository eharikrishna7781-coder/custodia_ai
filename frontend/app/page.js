...
import { cleanMarkdownFromText } from '@/lib/cleanText';
import {
  Send,
  Mic,
  Image as ImageIcon,
...
export default function Home() {
  ...
  const addMessage = (role, text, type = 'text') => {
    ...
    setMessages(prev => [...prev, { role, text: cleanedText, type, timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }]);
  };
  ...
  const handleVoiceInput = () => {
    ...
    if (!SpeechRecognition) {
      addMessage('bot', 'Voice input is not supported in this browser. Please type your symptoms.');
      return;
    }
    setIsListening(true);
  ...
  const clearHistory = () => {
    ...
    addMessage('bot', 'Symptom history and reminders cleared.');
  };
            ...
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center py-10">
                ...
                <div className="flex flex-wrap gap-2 mt-4 justify-center">
                  <button onClick={() => setInputText('I have a high fever and headache')} className={`text-xs px-4 py-2 rounded-full transition ${darkMode ? 'bg-slate-700 hover:bg-slate-600 text-slate-100' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'}`}>
                    Fever & headache
                  </button>
                  <button onClick={() => setInputText('I have chest pain and difficulty breathing')} className={`text-xs px-4 py-2 rounded-full transition ${darkMode ? 'bg-slate-700 hover:bg-slate-600 text-slate-100' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'}`}>
                    Chest pain
                  </button>
                  <button onClick={() => setInputText('I have a cough and cold')} className={`text-xs px-4 py-2 rounded-full transition ${darkMode ? 'bg-slate-700 hover:bg-slate-600 text-slate-100' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'}`}>
                    Cough & cold
                  </button>
                  <button onClick={() => setInputText('My child has a rash and fever')} className={`text-xs px-4 py-2 rounded-full transition ${darkMode ? 'bg-slate-700 hover:bg-slate-600 text-slate-100' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'}`}>
                    Child rash
                  </button>
                </div>
              </div>
            ...
            <div className={`rounded-2xl shadow-sm border p-4 sticky top-24 space-y-4 transition-colors ${
              ...
              {step === 2 && (
                <div className="space-y-2">
                  ...
                  <button onClick={() => handleTransportChoice('ambulance')} className="w-full flex items-center gap-2 px-4 py-3 bg-red-50 dark:bg-red-950/30 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-xl transition text-red-700 dark:text-red-400 font-medium">
                    <Ambulance className="w-5 h-5" /> Ambulance
                  </button>
                  <button onClick={() => handleTransportChoice('volunteer')} className="w-full flex items-center gap-2 px-4 py-3 bg-blue-50 dark:bg-blue-950/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-xl transition text-blue-700 dark:text-blue-400 font-medium">
                    <Car className="w-5 h-5" /> Volunteer Driver
                  </button>
                  <button onClick={() => { addMessage('bot', 'You chose your own transport. Route shown on map.'); goToStep(3); addMessage('bot', 'Follow the route on the map.', 'tracking'); }} className="w-full flex items-center gap-2 px-4 py-3 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-xl transition text-slate-700 dark:text-slate-300 font-medium">
